...
public class ProfessorController {
    ...
    public String criar(Model model, Professor professor) {
        ...
        model.addAttribute("cursos", this.geCursos());
        
        return "professores_criar";
    }

    private List<Curso> geCursos(){
    ...
    public String salvar(Professor professor, Integer cursoId){
      ...
      Curso cursoSelecionado = geCursos().stream().filter(c -> c.getCurso_id().equals(cursoId)).findFirst().orElse(new Curso(cursoId, "Curso inventado"));
